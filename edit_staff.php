...
if (isset($_POST["submit"])) {
    ...
    $query = mysqli_query($db, "UPDATE staf SET nama='$nama', telp='$telp', email='$email' WHERE id=$id");
    if ($query) {
...
// Jika ada pengiriman ID staff melalui URL
if (isset($_GET["id"])) {
    // Ambil ID staff dari URL
    $id = $_GET["id"];

    // Ambil data staff berdasarkan ID dari database
    $query_data = mysqli_query($db, "SELECT * FROM staf WHERE id=$id");
    $data_staff = mysqli_fetch_assoc($query_data);
} else {
    // Jika tidak ada ID staff, redirect ke halaman staff.php
